...
In each case, the additive default hides zeros inside averages: a country with zero press freedom scores 0.77 on democracy; a drug with a catastrophic safety profile receives a passing composite score; an engineering system with one failed component registers ``moderate risk.'' The multiplicative framework does not merely provide a better fit. It provides a different \textit{diagnosis}---one that surfaces the binding constraint rather than averaging it away.
